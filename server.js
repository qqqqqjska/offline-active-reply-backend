...
const SCREEN_USAGE_RETENTION_DAYS = 7;
...
CREATE TABLE IF NOT EXISTS screen_usage_devices (
  user_id TEXT NOT NULL,
  device_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS screen_usage_state (
  user_id TEXT NOT NULL,
  device_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_screen_usage_sessions_user_device_started ON screen_usage_sessions(user_id, device_id, started_at);
CREATE INDEX IF NOT EXISTS idx_screen_usage_sessions_user_device_ended ON screen_usage_sessions(user_id, device_id, ended_at);
`);
...
const getScreenUsageDeviceStmt = db.prepare(`SELECT * FROM screen_usage_devices WHERE user_id = ? AND device_id = ?`);
const insertScreenUsageDeviceStmt = db.prepare(`
INSERT INTO screen_usage_devices (user_id, device_id, secret_hash, created_at, updated_at)
VALUES (@user_id, @device_id, @secret_hash, @created_at, @updated_at)
`);
const getScreenUsageStateByAppStmt = db.prepare(`
SELECT * FROM screen_usage_state WHERE user_id = ? AND device_id = ? AND app_name = ?
`);
const listOpenScreenUsageStateStmt = db.prepare(`
SELECT * FROM screen_usage_state WHERE user_id = ? AND device_id = ? AND is_open = 1 ORDER BY updated_at DESC
`);
const upsertScreenUsageStateStmt = db.prepare(`
INSERT INTO screen_usage_state (user_id, device_id, app_name, is_open, last_opened_at, last_event_at, created_at, updated_at)
VALUES (@user_id, @device_id, @app_name, @is_open, @last_opened_at, @last_event_at, @created_at, @updated_at)
ON CONFLICT(user_id, device_id, app_name) DO UPDATE SET
  is_open=excluded.is_open,
  last_opened_at=excluded.last_opened_at,
  last_event_at=excluded.last_event_at,
  updated_at=excluded.updated_at
`);
const insertScreenUsageSessionStmt = db.prepare(`
INSERT INTO screen_usage_sessions (user_id, device_id, app_name, started_at, ended_at, duration_ms, created_at, updated_at)
VALUES (@user_id, @device_id, @app_name, @started_at, @ended_at, @duration_ms, @created_at, @updated_at)
`);
const listOpenScreenUsageSessionsByAppStmt = db.prepare(`
SELECT * FROM screen_usage_sessions
WHERE user_id = ? AND device_id = ? AND app_name = ? AND ended_at IS NULL
ORDER BY started_at DESC, id DESC
`);
const listOpenScreenUsageSessionsStmt = db.prepare(`
SELECT * FROM screen_usage_sessions
WHERE user_id = ? AND device_id = ? AND ended_at IS NULL
ORDER BY started_at DESC, id DESC
`);
const closeScreenUsageSessionStmt = db.prepare(`
UPDATE screen_usage_sessions
SET ended_at = @ended_at,
    duration_ms = @duration_ms,
    updated_at = @updated_at
WHERE id = @id
`);
const listScreenUsageSessionsForSummaryStmt = db.prepare(`
SELECT * FROM screen_usage_sessions
WHERE user_id = ?
  AND device_id = ?
  AND (
...
ORDER BY started_at DESC, id DESC
`);
const deleteOldScreenUsageSessionsStmt = db.prepare(`
DELETE FROM screen_usage_sessions WHERE user_id = ? AND device_id = ? AND started_at < ?
`);

function hashScreenUsageSecret(secret) {
    return crypto.createHash('sha256').update(String(secret || '').trim()).digest('hex');
}

function normalizeScreenUsageText(value) {
    return String(value || '').trim();
}

function normalizeScreenUsageAppName(value) {
    return normalizeScreenUsageText(value)
        .replace(/[\r\n]+/g, ' ')
        .replace(/\s{2,}/g, ' ')
        .slice(0, 80);
}

function getScreenUsageRequestAppName(req) {
    const pathAppName = typeof req?.params?.appName === 'string' ? req.params.appName : '';
    const queryAppName = typeof req?.query?.appName === 'string' ? req.query.appName : '';
    const rawValue = String(pathAppName || queryAppName || '');
    if (!rawValue) return '';
    try {
        return normalizeScreenUsageAppName(decodeURIComponent(rawValue));
    } catch (error) {
        return normalizeScreenUsageAppName(rawValue);
    }
}

function parseScreenUsageTimezoneOffsetMinutes(rawValue) {
    const parsed = Number(rawValue);
    return Number.isFinite(parsed) ? parsed : 0;
}

function getScreenUsageDayStartMs(now, timezoneOffsetMinutes = 0) {
    const offsetMinutes = parseScreenUsageTimezoneOffsetMinutes(timezoneOffsetMinutes);
    const shiftedNow = Number(now || Date.now()) - (offsetMinutes * 60000);
    const shiftedDate = new Date(shiftedNow);
    shiftedDate.setUTCHours(0, 0, 0, 0);
    return shiftedDate.getTime() + (offsetMinutes * 60000);
}

function formatScreenUsageDurationMs(durationMs) {
...
function cleanupScreenUsageHistory(userId, deviceId, now = Date.now()) {
    const cutoff = Number(now || Date.now()) - (SCREEN_USAGE_RETENTION_DAYS * 24 * 60 * 60 * 1000);
    deleteOldScreenUsageSessionsStmt.run(String(userId || 'default-user'), String(deviceId || 'default-device'), cutoff);
}

function ensureScreenUsageAuthorized(req, options = {}) {
    const userId = normalizeScreenUsageText(req.query.userId || 'default-user') || 'default-user';
    const deviceId = normalizeScreenUsageText(req.query.deviceId || 'default-device') || 'default-device';
    const secret = normalizeScreenUsageText(req.query.secret);
    const allowBootstrap = options && options.allowBootstrap !== false;

    if (!secret) {
        return { ok: false, status: 400, error: 'missing secret', userId, deviceId };
    }

    const existing = getScreenUsageDeviceStmt.get(userId, deviceId);
    const secretHash = hashScreenUsageSecret(secret);

    if (!existing) {
        if (!allowBootstrap) {
            return { ok: false, status: 404, error: 'device not found', userId, deviceId };
        }
        const now = Date.now();
        insertScreenUsageDeviceStmt.run({
            user_id: userId,
            device_id: deviceId,
        ...
        return { ok: true, userId, deviceId, bootstrapped: true };
    }

    if (String(existing.secret_hash || '') !== secretHash) {
        return { ok: false, status: 403, error: 'invalid secret', userId, deviceId };
    }

    return { ok: true, userId, deviceId, bootstrapped: false };
}

function buildScreenUsageRecentEvents(sessions, dayStartMs, now) {
    const events = [];
    (sessions || []).forEach((session) => {
        if (!session) return;
        const appName = String(session.app_name || 'Unknown App');
        const startedAt = Number(session.started_at || 0);
        const endedAt = session.ended_at === null || session.ended_at === undefined ? null : Number(session.ended_at || 0);
        const totalDurationMs = endedAt !== null ? Math.max(0, endedAt - startedAt) : Math.max(0, Number(now || Date.now()) - startedAt);
        if (startedAt >= dayStartMs) {
            events.push({
                type: 'open',
                appName,
                at: startedAt,
        ...
        if (endedAt !== null && endedAt >= dayStartMs) {
            events.push({
                type: 'close',
                appName,
                at: endedAt,
                durationMs: totalDurationMs,
                text: `Closed ${appName} after ${formatScreenUsageDurationMs(totalDurationMs)}`
            });
        }
    });
    return events
...
function buildScreenUsageSummary(userId, deviceId, options = {}) {
    const now = Number(options.now || Date.now());
    const timezoneOffsetMinutes = parseScreenUsageTimezoneOffsetMinutes(options.timezoneOffsetMinutes);
    const dayStartMs = getScreenUsageDayStartMs(now, timezoneOffsetMinutes);
    const sessions = listScreenUsageSessionsForSummaryStmt.all(userId, deviceId, dayStartMs, dayStartMs);
    ...
    sessions.forEach((session) => {
        ...
        const startedAt = Number(session.started_at || 0);
        const endedAt = session.ended_at === null || session.ended_at === undefined ? null : Number(session.ended_at || 0);
        const overlapStart = Math.max(startedAt, dayStartMs);
        const overlapEnd = Math.min(endedAt === null ? now : endedAt, now);
        ...
        entry.totalDurationMs += overlapDurationMs;
        if (startedAt >= dayStartMs) {
            entry.openCount += 1;
        }
        if (!entry.lastOpenedAt || startedAt > entry.lastOpenedAt) {
            entry.lastOpenedAt = startedAt;
        }
        if (endedAt !== null && (!entry.lastClosedAt || endedAt > entry.lastClosedAt)) {
            entry.lastClosedAt = endedAt;
        }
        if (endedAt === null) {
            entry.isOpen = true;
            entry.currentDurationMs = Math.max(0, now - startedAt);
            if (!currentApp || startedAt > Number(currentApp.startedAt || 0)) {
                currentApp = {
                    appName,
                    startedAt,
        ...
        totalScreenTimeMs += overlapDurationMs;
    });

    const apps = Array.from(appMap.values())
        .sort((left, right) => {
            const durationDiff = Number(right.totalDurationMs || 0) - Number(left.totalDurationMs || 0);
            if (durationDiff !== 0) return durationDiff;
            return Number(right.lastOpenedAt || 0) - Number(left.lastOpenedAt || 0);
        });

    const recentEvents = buildScreenUsageRecentEvents(sessions, dayStartMs, now);
    ...
    return {
        ...
        dayStartAt: dayStartMs,
        totalScreenTimeMs,
        totalScreenTimeText: formatScreenUsageDurationMs(totalScreenTimeMs),
        currentApp,
        apps,
        recentEvents
    };
...
const toggleScreenUsageStateTransaction = db.transaction(({ userId, deviceId, appName, now }) => {
    const existingState = getScreenUsageStateByAppStmt.get(userId, deviceId, appName);
    const openSessions = listOpenScreenUsageSessionsByAppStmt.all(userId, deviceId, appName);
    const isCurrentlyOpen = !!((existingState && Number(existingState.is_open) === 1) || openSessions.length > 0);

    if (isCurrentlyOpen) {
        openSessions.forEach((session) => {
            const startedAt = Number(session && session.started_at ? session.started_at : now);
            closeScreenUsageSessionStmt.run({
                id: session.id,
                ended_at: now,
        ...
        upsertScreenUsageStateStmt.run({
            user_id: userId,
            device_id: deviceId,
            app_name: appName,
            is_open: 0,
            last_opened_at: existingState && existingState.last_opened_at ? Number(existingState.last_opened_at) : (openSessions[0] ? Number(openSessions[0].started_at || now) : now),
            last_event_at: now,
            created_at: existingState && existingState.created_at ? Number(existingState.created_at) : now,
            updated_at: now
        });
        cleanupScreenUsageHistory(userId, deviceId, now);
        return { state: 'closed', appName, at: now };
    }
    ...
    insertScreenUsageSessionStmt.run({
        user_id: userId,
        device_id: deviceId,
        app_name: appName,
        started_at: now,
    ...
    cleanupScreenUsageHistory(userId, deviceId, now);
    return { state: 'open', appName, at: now };
});

const resetAllScreenUsageTransaction = db.transaction(({ userId, deviceId, now }) => {
    const openSessions = listOpenScreenUsageSessionsStmt.all(userId, deviceId);
    const openStates = listOpenScreenUsageStateStmt.all(userId, deviceId);
    const closedApps = new Set();
    ...
    openSessions.forEach((session) => {
        ...
        closedApps.add(String(session.app_name || 'Unknown App'));
    });

    openStates.forEach((stateRow) => {
        const appName = String(stateRow.app_name || 'Unknown App');
        upsertScreenUsageStateStmt.run({
            user_id: userId,
            device_id: deviceId,
            app_name: appName,
            is_open: 0,
            last_opened_at: stateRow.last_opened_at ? Number(stateRow.last_opened_at) : now,
            last_event_at: now,
            created_at: stateRow.created_at ? Number(stateRow.created_at) : now,
            updated_at: now
        });
    ...
    cleanupScreenUsageHistory(userId, deviceId, now);
    return { state: 'reset', closedApps: Array.from(closedApps.values()) };
});
...
function handleScreenUsageToggle(req, res) {
    const auth = ensureScreenUsageAuthorized(req, { allowBootstrap: true });
    if (!auth.ok) {
        return res.status(auth.status || 400).json({ ok: false, error: auth.error || 'unauthorized' });
    }

    const appName = getScreenUsageRequestAppName(req);
    if (!appName) {
        return res.status(400).json({ ok: false, error: 'missing appName' });
    }

    const now = Date.now();
    const timezoneOffsetMinutes = parseScreenUsageTimezoneOffsetMinutes(req.query.tzOffsetMinutes);
    const toggled = toggleScreenUsageStateTransaction({
        userId: auth.userId,
        deviceId: auth.deviceId,
        appName,
        now
    });
    const summary = buildScreenUsageSummary(auth.userId, auth.deviceId, { now, timezoneOffsetMinutes });
    const appSummary = (summary.apps || []).find((item) => String(item.appName || '') === appName) || {
        appName,
        totalDurationMs: 0,
        openCount: 0,
        lastOpenedAt: toggled.state === 'open' ? now : null,
        lastClosedAt: toggled.state === 'closed' ? now : null,
        isOpen: toggled.state === 'open',
        currentDurationMs: toggled.state === 'open' ? 0 : 0
    };
    ...
    return res.json({
        ...
        generatedAt: now,
        timezoneOffsetMinutes,
        totalDurationMs: Number(appSummary.totalDurationMs || 0),
        totalDurationText: formatScreenUsageDurationMs(appSummary.totalDurationMs || 0),
        openCount: Number(appSummary.openCount || 0),
        currentApp: summary.currentApp,
        totalScreenTimeMs: Number(summary.totalScreenTimeMs || 0),
        totalScreenTimeText: summary.totalScreenTimeText || formatScreenUsageDurationMs(summary.totalScreenTimeMs || 0)
    });
}

app.get(['/api/screentime/toggle', '/api/screentime/toggle/:appName'], handleScreenUsageToggle);

app.get('/api/screentime/summary', (req, res) => {
    const auth = ensureScreenUsageAuthorized(req, { allowBootstrap: true });
    if (!auth.ok) {
        return res.status(auth.status || 400).json({ ok: false, error: auth.error || 'unauthorized' });
    }
    const now = Date.now();
    const timezoneOffsetMinutes = parseScreenUsageTimezoneOffsetMinutes(req.query.tzOffsetMinutes);
    cleanupScreenUsageHistory(auth.userId, auth.deviceId, now);
    const summary = buildScreenUsageSummary(auth.userId, auth.deviceId, { now, timezoneOffsetMinutes });
    return res.json({
        ok: true,
        bootstrapped: !!auth.bootstrapped,
...
app.get('/api/screentime/reset-all', (req, res) => {
    const auth = ensureScreenUsageAuthorized(req, { allowBootstrap: true });
    if (!auth.ok) {
        return res.status(auth.status || 400).json({ ok: false, error: auth.error || 'unauthorized' });
    }
    const now = Date.now();
    const timezoneOffsetMinutes = parseScreenUsageTimezoneOffsetMinutes(req.query.tzOffsetMinutes);
    const resetResult = resetAllScreenUsageTransaction({
        userId: auth.userId,
        deviceId: auth.deviceId,
        now
    });
    ...
    return res.json({
        ...
        closedApps: resetResult.closedApps || [],
        summary
    });
});
